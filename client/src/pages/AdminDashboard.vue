      ...
      <div class="bg-slate-900 p-5 md:p-6 rounded-3xl border border-slate-800 active:scale-95 md:hover:scale-[1.02] transition-all md:hover:border-green-500/50 transition-all cursor-pointer">
        <div class="flex justify-between items-start mb-2">
           ...
           <div class="text-right">
             <p class="text-[8px] font-black text-slate-500 uppercase tracking-widest text-emerald-400">Total Interest: {{ loading ? '...' : formatCurrency(stats.lending_sales) }}</p>
           </div>
        </div>
        ...
        <div class="pt-4 border-t border-slate-800/50">
          <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest text-rose-400">Total Accomulated: {{ formatCurrency(stats.total_accumulate) }}</p>
        </div>
      </div>
    </div>
